...
class BigDTConfig extends Config(
  new WithDTAccelerator(64, bigConfig) ++ new WithComposer() ++ new WithAWSPlatform(1)
)

class BiggerDTConfig extends Config(
  new WithDTAccelerator(96, bigConfig) ++ new WithComposer() ++ new WithAWSPlatform(1)
)

class SmallDTConfig extends Config(
  new WithDTAccelerator(4, smallConfig) ++ new WithComposer() ++ new WithAWSPlatform(1)
)

object DTDriver extends App {
  composer.TestDriver.buildConfig(new BiggerDTConfig)
}
