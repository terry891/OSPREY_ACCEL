		...
		for (Atom[] pair : atomPairs) {
			if (pair[0].res != res1 || pair[1].res != res2) {
				throw new RuntimeException(
						"ERROR: Sparse forcefield energy given atom pairs with inconsistent residues");
			}
		}
		...
		if (intra) {
			...
			for (int atNum = 0; atNum < atoms1.size(); atNum++) {
				if (atoms1.get(atNum) != atoms2.get(atNum))
					throw new RuntimeException(
							"ERROR: Atoms for intra-residue energy defined inconsistently");
...
//////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void initializeCalculation(List<Atom> atoms1, List<Atom> atoms2) {
		...
		initializeEVCalculation(pairs14,
				pairsNonBonded); //initialize the calculation of the electrostatic and vdW terms
	...
	private void initializeEVCalculation(List<Atom[]> pairs14, List<Atom[]> pairsNonBonded) {
		...
		for (int i = 0; i < number14Connections; i++) {
			...
			if (!(params.getNonBondedParameters(res1.atoms.get(atom1), nbparams)))
				System.out.println(
						"WARNING: Could not find nb parameters for " + atom1 + " type: " + res1.atoms.get(
								atom1).forceFieldType);
			else {
				...
				if (!(params.getNonBondedParameters(res2.atoms.get(atom4), nbparams)))
					System.out.println(
							"WARNING: Could not find nb parameters for " + atom4 + " type: " + res2.atoms.get(
									atom1).forceFieldType);
				else {
					if ((params.forcefld == ForcefieldParams.Forcefield.CHARMM19
							|| params.forcefld == ForcefieldParams.Forcefield.CHARMM19NEUTRAL)
		...
		for (int i = 0; i < numberNonBondedConnections; i++) {
			...
			if (!(params.getNonBondedParameters(res1.atoms.get(atom1), nbparams)))
				System.out.println(
						"WARNING: Could not find nb parameters for (at1) " + atom1 + " type: " + res1.atoms.get(
								atom1).forceFieldType);
			else {
				double epsilonProduct = nbparams.epsilon;
				double ri = nbparams.r;
				if (!(params.getNonBondedParameters(res2.atoms.get(atom2), nbparams)))
					System.out.println("WARNING: Could not find nb parameters for (at2) " + atom2 + " type: "
							+ res2.atoms.get(atom2).forceFieldType);
				else {
					epsilonProduct *= nbparams.epsilon;
...
//global
	...
	private double checkEnergy(double energy) {
		if (Double.isNaN(energy)) {
			...
			throw new RuntimeException(
					"ERROR: NaN returned by ForcefieldEnergy.  No NaN or infinite coordinates.");
		}
